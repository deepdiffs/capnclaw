...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // Docker supports file mounts (mount /dev/null over .env). Apple Container does
    // not — it falls back to mount --bind inside the entrypoint.
...
function buildContainerArgs(
  ...
  const { PARALLEL_API_KEY } = readEnvFile(['PARALLEL_API_KEY']);
  if (PARALLEL_API_KEY) {
